...
are losses to a more-popular hard negative; score--popularity correlation up to
$\rho=1.0$) and extreme patent cold-start (91.7\% of test patents unseen; SVD
...
perfect scores, which average-rank tie-breaking removes. A controlled random-split
contrast shows the protocol is decisive: a random split lowers cold-start to 2.4\%
and makes SVD the single best method (NDCG@10\,=\,0.442, AUC\,=\,0.830), the exact
reverse of the temporal verdict. Our contributions are the evaluation protocol, a
suite of bias and cold-start diagnostics, and a symmetric grid of mitigation
baselines that yield only partial, unstable gains.
\end{abstract}
...
\item A random split reverses the verdict (SVD becomes best at 0.442), showing the evaluation protocol is decisive.
...
data reports Recall@5\,=\,0.998 and NDCG@5\,=\,0.997 \cite{kim2025} --- results that,
as we show, do not survive a leakage-free, same-IPC hard-negative re-evaluation.
...
NDCG@10\,=\,0.197; an IPC-conditional skyline reaches 0.199; and NGCF --- which our
...
98.8\% of GraphSAGE and 57.8\% of GAT failures are losses to a more-popular same-IPC
hard negative; the hard-negative inversion rate is 56\% for MostPop and NGCF and 28\%
...
candidates and appear near-perfect (\S\ref{sec:tiebreak}); an average-rank
tie-break removes this artifact. These observations motivate three research
questions answered in the paper:
\begin{itemize}
...
\noindent Our contributions are: (i) a realistic evaluation protocol for the task on
KIPRIS; (ii) a diagnosis that GNNs, CF, MF, structural heuristics, and a text MLP all
fall below popularity baselines with AUC near chance; (iii) a suite of
bias-diagnostic tools that localize the failure; (iv) a mitigation grid showing only
partial gains; and (v) the strict-tie-break artifact and its average-rank correction,
corroborated by an unsampled full-pool metric and a temporal-vs-random contrast. The
aim is not a winning model but an evaluation protocol and diagnosis to discipline
future work. Figure~\ref{fig:overview} gives an overview.
...
non-learned baselines. The recurring theme is that our contribution is diagnostic:
...
LightGCN/NGCF use a learnable per-company identity embedding ($d=64$). An encoder
...
decoder scores a pair by $s(p,c)=h_p^{\top}h_c$. For a held-out test transfer
...
We evaluate nine base models --- three non-learning skylines (\textbf{MostPop},
IPC-conditional \textbf{MostPop-IPC}, \textbf{Recency}), two structural heuristics
(\textbf{Common Neighbors}, \textbf{Adamic--Adar}), \textbf{SVD} ($k{=}64$), a
text-only \textbf{MLP}, and two CF models (\textbf{LightGCN}, \textbf{NGCF}) ---
together with two heterogeneous GNNs (\textbf{GraphSAGE}, \textbf{GAT}), and a
...
attention weights for diagnostics. Training minimizes a binary objective over
observed transfers and sampled negatives, with early stopping on validation NDCG@10
(patience 5).
...
(average padding rate 1.07\% when fewer than 100 exist).
...
resolved by \textbf{average rank}: a candidate that ties the positive contributes 0.5
to its rank. This is essential --- under a strict greater-than rule a no-information
model whose candidates all tie places the positive first by construction and scores
near-perfect (\S\ref{sec:tiebreak}).
...
are non-learned or popularity-equivalent. Full 25-row table (with standard
...
and the candidates' training popularity is 1.0 for MostPop and \textbf{0.96 for NGCF}
...
ranked above the true positive in \textbf{56.4\%} of MostPop and \textbf{56.3\%} of
NGCF queries, versus \textbf{28.4\%} for GAT. (iii) A Mann--Whitney U test comparing
...
(NDCG@10\,=\,0.016) and best on the unpopular tail (0.370) --- the opposite of a
popularity-exploiting model, yet its absolute accuracy stays far below MostPop because
the head holds most positives (Figure~\ref{fig:stratified}).
...
\label{sec:coldstart}
Under the temporal split, \textbf{91.7\% of test patents are unseen in training}.
Decomposing accuracy by patent novelty is decisive for SVD: on the new-patent subset
its NDCG@10 is \textbf{0.000} (its latent factor for an unseen patent is zero), whereas
...
shows the backbones fail differently (Figure~\ref{fig:error}). For \textbf{GraphSAGE,
98.8\%} of failures are losses to a more-popular hard negative (0.1\% rare/new, 1.1\%
residual): it fails almost entirely through popularity. For \textbf{GAT, 57.8\% /
8.3\% / 33.9\%} --- popularity still dominates, but a third of GAT's errors are a
semantic residual, consistent with its near-zero popularity correlation. Both fail;
they fail by different mechanisms.
...
construction. Replacing the strict rule with an \textbf{average-rank} tie-break (a tie
...
contrasts the rules. We regard this as a generic pitfall for sampled-ranking
...
lowers the patent cold-start rate from \textbf{91.7\% to 2.4\%}, because patents are
scattered across train and test rather than held out by time. Under it,
\textbf{matrix factorization becomes the single strongest method}: SVD rises from
NDCG@10\,=\,0.037 (AUC 0.52) to \textbf{0.442 (AUC 0.830)}, comfortably exceeding
MostPop (0.219) --- the exact reverse of the temporal verdict. AUC inflates across the
board (MostPop $0.583\!\to\!0.667$; GAT $0.519\!\to\!0.650$). The mechanism is
transparent: with only 2.4\% new patents, SVD's latent factors (useless on
...
learned-model win; the realistic protocol, by exposing the cold-start regime the
random split conceals, overturns it. This is the empirical core of RQ1 and the reason
...
popularity-debiased negative sampling (exponent $\alpha$), IPS / log-popularity
re-ranking (penalty $\beta$), the logQ correction, DropEdge, a sinusoidal
transfer-edge time encoding, and the stacked combinations Debias+IPS and Time+IPS. The
gains are \textbf{partial and unstable} (Figure~\ref{fig:sweeps}).
...
$\approx$0.157--0.158 --- but at the cost of driving \textbf{AUC to 0.417, below
...
\paragraph{Single chronological cutoff.} We use a single 70/15/15 temporal split and do
not perform rolling-origin evaluation, so stability across cutoffs is unverified. The
controlled random-split contrast (\S\ref{sec:rq1}) nonetheless confirms that the
realistic protocol is what exposes the failure.
...
factorization becomes the single best method (0.442, AUC 0.830), the exact reverse of
...
The KIPRIS patent data is obtained from the Korea Intellectual Property Rights
Information Service API (\url{https://plus.kipris.or.kr}); access requires a free API
key, and redistribution of the raw exports is subject to KIPRIS terms. We release the
preprocessing and evaluation code, the experiment scripts, and a script to regenerate
the frozen SBERT embeddings from patent text, enabling full reproduction of all tables
and figures from the KIPRIS exports.
